...
public class Solution {
	...
	public static void main(String[] args){
		Pattern p1 = Pattern.compile("^([a-z]+) (AND|OR) ([a-z]+) -> ([a-z]+)");
		Pattern p2 = Pattern.compile("^([a-z]+) (L|R)SHIFT ([0-9]+) -> ([a-z]+)");
		Pattern p3 = Pattern.compile("^NOT ([a-z]+) -> ([a-z]+)");

		// Generate a mapping of wires to the command setting the wire
		TreeMap<String, String> cmds = new TreeMap<>();
		TreeMap<String, Integer> knownWires = new TreeMap<>();
		for(String s: INPUT){
			String[] tmp = s.split(" -> ");
			cmds.put(tmp[1], tmp[0]);
		}

		int result = valueOfWire("a", cmds, knownWires);
		...
		// Overwrite wire b
		cmds.put("b", "" + result);
		// Empty the known wires
		knownWires = new TreeMap<>();
		result = valueOfWire("a", cmds, knownWires);

		System.out.println("Part 2 - Success");
		System.out.println("Answer\t" + result);
	}

	// Way to complex, but it works
	public static int valueOfWire(String wire, TreeMap<String, String> cmds, TreeMap<String, Integer> known){
		if(known.containsKey(wire))
			return known.get(wire);

		int result = 0;

		String cmd = cmds.get(wire);
		if(cmd.contains("AND") || cmd.contains("OR")){

			Pattern p = Pattern.compile("^([a-z0-9]+) (AND|OR) ([a-z]+)");
			Matcher m = p.matcher(cmd);
			m.find();

			int w1 = (isInteger(m.group(1)) ? Integer.parseInt(m.group(1)) : valueOfWire(m.group(1), cmds, known));
			int w2 = (isInteger(m.group(3)) ? Integer.parseInt(m.group(3)) : valueOfWire(m.group(3), cmds, known));

			if(m.group(2).equals("AND"))
				result = BitwiseGates.AND(w1, w2);
			else
				result = BitwiseGates.OR(w1, w2);
		}
		else if(cmd.contains("LSHIFT") || cmd.contains("RSHIFT")){
			Pattern p = Pattern.compile("^([a-z0-9]+) (L|R)SHIFT ([0-9]+)");
			Matcher m = p.matcher(cmd);
			m.find();

			int w = (isInteger(m.group(1)) ? Integer.parseInt(m.group(1)) : valueOfWire(m.group(1), cmds, known));
			int shift = Integer.parseInt(m.group(3));

			if(m.group(2).equals("L"))
				result = BitwiseGates.LSHIFT(w, shift);
			else
				result = BitwiseGates.RSHIFT(w, shift);
		}
		else if(cmd.contains("NOT")){
			Pattern p = Pattern.compile("^NOT ([a-z0-9]+)");
			Matcher m = p.matcher(cmd);
			m.find();

			int w = (isInteger(m.group(1)) ? Integer.parseInt(m.group(1)) : valueOfWire(m.group(1), cmds, known));

			result = BitwiseGates.NOT(w);
		}
		else{
			try {
				...
				result = valueOfWire(cmd, cmds, known);
			}
		}

		known.put(wire, result);
		return result;
	}

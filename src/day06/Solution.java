...
public class Solution {
	...
	public static void main(String[] args){
		...
		for(String s: INPUT){
			...
			if(cmd.equals("turn on"))
				mode = 1;
			else if(cmd.equals("turn off"))
				mode = 2;
			else
				mode = 3;

			setBlock(x1, y1, x2, y2, mode, grid);
			setBlock2(x1, y1, x2, y2, mode, grid2);
		}
	...
	private static final void setBlock(int x1, int y1, int x2, int y2, int mode, boolean[][] grid){
		for(int y = y1; y <= y2; y++){
			for(int x = x1; x <= x2; x++){
				if(mode == 1)
					grid[x][y] = true;
	...
	private static final void setBlock2(int x1, int y1, int x2, int y2, int mode, int[][] grid) {
		for(int y = y1; y <= y2; y++){
			for(int x = x1; x <= x2; x++){
				if(mode == 1)
					grid[x][y]++;
				else if(mode == 2 && grid[x][y] > 0) {
					grid[x][y]--;
				}
				else if(mode == 3)
					grid[x][y] += 2;

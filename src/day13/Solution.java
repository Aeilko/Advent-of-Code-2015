...
public class Solution {
	...
	public static void main(String[] args){
		Pattern pat = Pattern.compile("([A-Za-z]+) would (lose|gain) ([0-9]+) happiness units by sitting next to ([A-Za-z]+).");
		...
		for(String s: INPUT){
			...
			int score = Integer.parseInt((tmp.equals("lose") ? "-" : "") + m.group(3));

			scores.put(name1 + name2, score);
			names.add(name1);
		}

		int max = getMaxScore(names, scores);
		...
		max = getMaxScore(names, scores);

		System.out.println("Part 2 - Success");
		System.out.println("Answer\t" + max);
	}

	public static int getScore(String[] seating, TreeMap<String, Integer> scores){
		int result = 0;

		for(int i = 0; i < seating.length; i++){
			if(i == 0)
				result += scores.get(seating[i] + seating[seating.length-1]);
			else
				result += scores.get(seating[i] + seating[i-1]);

			if(i == seating.length-1)
				result += scores.get(seating[i] + seating[0]);
			else
				result += scores.get(seating[i] + seating[i+1]);
		}
	...
	public static int getMaxScore(TreeSet<String> names, TreeMap<String, Integer> scores){
		int result = 0;

		String[] stringArray = new String[names.size()];
		names.toArray(stringArray);
		ArrayList<String[]> perms = Permutations.getAll(stringArray);
		for(String[] sa: perms){
			int s = getScore(sa, scores);
			if(s > result)
				result = s;
		}

...
public class Solution {
	...
	public static void main(String[] args){
		...
		// TreeSets wont allow duplicates, so this should work fine
		TreeSet<Coordinate> coords = new TreeSet<>();
		TreeSet<Coordinate> coords2 = new TreeSet<>();
		int combX, santX, roboX;
		combX = santX = roboX = 0;
		int combY, santY, roboY;
		combY = santY = roboY = 0;

		coords.add(new Coordinate(combX, combY));
		coords2.add(new Coordinate(santX, santY));
		...
		for(char c: input.toCharArray()){
			...
			if(i%2 == 0)
				...
				coords2.add(new Coordinate(roboX, roboY));
			i++;
		}

		System.out.println("Part 1 - Success");
		System.out.println("Answer\t" + coords.size() + "\n");

		System.out.println("Part 2 - Success");
		System.out.println("Answer\t" + coords2.size());
	}
}
